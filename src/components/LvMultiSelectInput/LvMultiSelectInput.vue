<template>
  <div :class="rootClasses" ref="rootRef">
    ...
    >
      ...
      <LvBaseIcon :name="open ? 'chevron-up' : 'chevron-down'" size="sm" aria-hidden="true" />
    </button>
    ...
    >
      ...
      <div class="lv-multi-select__search">
        <LvBaseIcon name="search" size="sm" class="lv-multi-select__search-icon" aria-hidden="true" />
        <input
          ref="searchRef"
          v-model="searchQuery"
          type="text"
      ...
      <ul class="lv-multi-select__list" role="presentation">
        ...
        >
          <span class="lv-multi-select__option-check" aria-hidden="true">
            <LvBaseIcon v-if="isSelected(opt.value)" name="check" size="xs" />
          </span>
          {{ opt.label }}
        </li>
...
/**
 * LvMultiSelectInput — Custom multi-select dropdown with search filter.
